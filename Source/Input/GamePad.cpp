...
void GamePad::update()
{
	...
	// キーボードにエミュレーション
	{
		...
		if(GetAsyncKeyState('5') & 0x8000)	newButtonState |= BTN_5;
		if(GetAsyncKeyState('6') & 0x8000)	newButtonState |= BTN_6;
		if(GetAsyncKeyState('7') & 0x8000)	newButtonState |= BTN_7;
		if(GetAsyncKeyState('8') & 0x8000)	newButtonState |= BTN_8;
